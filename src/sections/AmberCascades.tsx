...
function initAmberCascades(canvas: HTMLCanvasElement) {
  ...
  // Hardware/circuit themed symbols in green
  const hardwareSymbols = '{}[]()<>;:=+-*/|&~^%#@!?_\\';
  const codeChars = '01ABCDEFxVCCGNDINOUTPWMUARTSPII2C';
  const allChars = hardwareSymbols + codeChars;
  ...
  function render(timestamp: number) {
    ...
    for (const col of columns) {
      ...
      for (let j = 0; j < col.length; j++) {
        ...
        // Green color palette instead of amber
        let r: number, g: number, b: number;
        if (j === 0) { r = 110; g = 231; b = 183; } // bright emerald
        else if (j < 3) { r = 52; g = 211; b = 153; } // medium emerald
        else { r = 16; g = 185; b = 129; } // base emerald
    ...
    waterGrad.addColorStop(0, 'rgba(10, 20, 15, 0.6)');
    ...
    for (const r of ripples) {
      ...
      for (let ring = 0; ring < 3; ring++) {
        ...
        ctx.strokeStyle = `rgba(16, 185, 129, ${alpha * (1 - ring * 0.3)})`;
